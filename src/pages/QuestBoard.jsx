...
import { 
  ArrowLeft, CheckCircle, Coins, Star, Brain, Zap, AlertTriangle, 
  Upload, Clock, BookText, MessageSquare, Swords, Palette, Heart, Wand, Flame 
} from 'lucide-react';
...
const getQuestBanner = (quest) => {
  if (quest.type === 'upload') return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/herald.post.banner.png";
  if (quest.id === 103) return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/The.Hunters.Archives.banner2.png";
  if (quest.id === 104) return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/The.Sunken.Palace.png";
  if (quest.id === 106 || quest.type === 'journal') return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/daily.journal.banner.png";
  if (quest.id === 110 || quest.type === 'wellness') return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/Tavern.rest.background.png";
  if (quest.id === 112) return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/Science.quiz.banner2.png";
  if (quest.id === 113) return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/Scribe.gloss.banner.png";
  if (quest.id === 114) return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/Spellforge.banner2.png";
  if (quest.type === 'multi-step') return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/The.Sunken.Lagoon.png";
  if (quest.type === 'incantation') return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/The.Haunted.Scriptorium.png";
  if (quest.type === 'scout-sports') return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/The.Proving.Grounds.png";
  if (quest.type === 'scout-arts') return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/The.Grand.Studio.png";
  if (quest.type === 'gauntlet') return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/Gaunlet.dojo.banner.png";
  return "https://cdn.jsdelivr.net/gh/w1zinvestmentss-star/game-assets@main/worldmap4.png";
};
...
const QuestBoard = () => {
  ...
  const { 
    quests, submitQuest, getQuestStatus, currentUser, attemptQuiz, 
    attemptScenario, submitWellnessCheck, globalEffects, resolveVoidGrasp, 
    getHighScore 
  } = useGame();
  ...
  const getVoidGraspTimeLeft = () => {
    ...
    const expirationTime = createdTime + 86400000;
  ...
  const handleFileSelect = (e) => {
    ...
    if (file && questId) {
      // Memory / Safety check: 15MB limit
  ...
  return (
    <div className="min-h-screen text-stone-200 p-6 relative select-none">
      ...
      <div className="relative z-10 pt-12 sm:pt-14 md:pt-4">
        ...
        <div className="max-w-3xl mx-auto">
          <h1 className="text-4xl text-yellow-400 mb-10 text-center font-['Press_Start_2P'] drop-shadow-[0_2px_10px_rgba(234,179,8,0.4)]">
            QUEST BOARD
          </h1>
          ...
          {isLocked ? (
            <div className="bg-purple-900/80 border-4 border-purple-500 p-8 rounded-xl max-w-2xl mx-auto text-center shadow-[0_0_50px_rgba(168,85,247,0.5)]">
              ...
              <button 
                ...
                className="w-full bg-yellow-500 text-black py-4 rounded font-bold font-['VT323'] text-3xl hover:bg-yellow-400 transition-colors cursor-pointer"
              >
                BREAK SEAL
              </button>
            </div>
            ...
            <div>
              {Object.keys(QUEST_CATEGORIES).map(categoryKey => {
                ...
                if (activeSector === 'uploads' && !['upload', 'quiz', 'scout-sports', 'scout-arts'].includes(categoryKey)) return null;
                  ...
                  <div key={categoryKey} className="mb-12">
                    ...
                    <div className="grid gap-4">
                      {categoryQuests.map((quest) => {
                        ...
                        const bgArt = getQuestBanner(quest);
                        ...
                        return (
                          <motion.div 
                            ...
                            className={`relative group overflow-hidden rounded-lg border-2 ${
                              status === 'approved' || status === 'read_only' 
                                ? 'border-green-600' 
                                : status === 'failed' 
                                ? 'border-red-800' 
                                : theme.frameBorder
                          ...
                          >
                            ...
                            <img 
                              src={bgArt} 
                              alt={quest.title} 
                              className="absolute right-0 top-0 bottom-0 w-1/2 h-full object-cover object-right opacity-55 pointer-events-none z-0" 
                            />
                                ...
                                {status === 'available' ? (
                                  quest.type === 'incantation' && quest.questionBank?.length > 0 ? (
                                    ...
                                    <button 
                                      ...
                                      className={`inline-flex items-center justify-center gap-2 px-5 py-3 w-auto min-w-fit rounded-lg font-['Press_Start_2P'] text-[10px] tracking-wider uppercase whitespace-nowrap transition-all duration-100 active:translate-y-[2px] active:shadow-[0_1px_0_#000] ${theme.buttonBg}`}
                                    >
                                      ...
                                      <div className="space-y-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                          <input
                                            ...
                                            onKeyDown={(e) => e.key === 'Enter' && handleActiveQuizSubmit(quest.id)}
                                    ...
                                    quest.id === 106 ? (
                                      <button onClick={() => navigate('/briefing/106')} className={`inline-flex items-center justify-center gap-2 px-5 py-3 w-auto min-w-fit rounded-lg font-['Press_Start_2P'] text-[10px] tracking-wider uppercase whitespace-nowrap transition-all duration-100 active:translate-y-[2px] active:shadow-[0_1px_0_#000] ${theme.buttonBg}`}>
                                        <BookText size={18} /> SPEAK TO SCRIBE
                                      </button>
                                    ) : (
                                      <div className="flex flex-col items-end gap-2">
                                        <textarea placeholder="Write your reflection..." value={journalTexts[quest.id] || ''} onChange={(e) => handleJournalTextChange(quest.id, e.target.value)} className="bg-black/80 border border-stone-600 rounded-md p-2 w-full h-24 text-stone-200 font-mono focus:ring-1 focus:ring-blue-500" />
                                        <button onClick={() => handleJournalSubmit(quest.id)} className="px-4 py-2 bg-blue-700 text-white rounded-lg hover:bg-blue-600 flex items-center gap-2 font-['VT323'] text-xl"><Upload size={18} /> SUBMIT</button>
                                      </div>
                                    )
                                  ) : quest.type === 'upload' ? (
                                    <button onClick={() => navigate('/briefing/' + quest.id)} className={`inline-flex items-center justify-center gap-2 px-5 py-3 w-auto min-w-fit rounded-lg font-['Press_Start_2P'] text-[10px] tracking-wider uppercase whitespace-nowrap transition-all duration-100 active:translate-y-[2px] active:shadow-[0_1px_0_#000] ${theme.buttonBg}`}>
      ...
      {showVictoryModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 font-['VT323']">
          <div className="bg-stone-900 border-2 border-yellow-500 rounded-lg p-8 text-center max-w-sm mx-auto">
            ...
            <button onClick={() => setShowVictoryModal(false)} className="px-6 py-2 bg-yellow-600 text-black rounded-lg hover:bg-yellow-500 shadow-lg border-2 border-yellow-400 text-xl font-bold cursor-pointer">Huzzah!</button>
          </div>
        </div>
      )}
    </div>
